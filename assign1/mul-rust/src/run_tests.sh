...
# Arguments
MODES_RAW=$1  # Can be a single number or a list like [1,2,3]
ITER=$2       # Number of iterations (each iteration runs a fresh execution)
PN=$3         # "p" for parallel, "n" for normal (only for mode 2)
BLOCK_SIZE=$4 # Block size (only for mode 3)

# Remove brackets from the modes list and convert it to an array
...
# Compile the program if necessary using cargo build --release
EXECUTABLE="./target/release/multiplication"
...
# If any mode is 4, execute all tests by recursively calling this script
if [[ "$MODES" =~ "4" ]]; then
...
for MODE in $MODES; do
    ...
    # Define parallel flag (applies only to mode 2)
    PARALLEL_FLAG=0
    if [[ "$MODE" -eq 2 ]]; then
        if [ "$PN" == "p" ]; then
            PARALLEL_FLAG=1
    ...
    # Define subdirectory name based on the mode within the test folder
    if [[ "$MODE" -eq 1 ]]; then
        SUBDIR="basic"
    elif [[ "$MODE" -eq 2 ]]; then
    ...
    # Create subdirectory inside the test directory
    ...
    for SIZE in "${MATRIX_SIZES[@]}"; do
        ...
        echo "Executing mode=$MODE for matrix of size $SIZE with $ITER iterations..."
        # Remove previous output file, if it exists
        rm -f "$OUTPUT_FILE"

        # For each iteration, run the Rust binary with a single iteration (to keep cache cold)
        for ((i=1; i<=ITER; i++)); do
            echo "Iteration $i..."
            if [[ "$MODE" -eq 3 ]]; then
                if [ $i -eq 1 ]; then
